...
.Parameter PreTestBlock
    Insert this code block before the main test query - usually contains a WITH clause.
.Example
...
process {
    ...
    # Try to match the full test name with the case of the object name
    if ( -not ($ObjectName -cmatch '[a-z]') ) {
        # ALL_CAPS
        $TestName = "$($ObjectName)_TESTS"
    } elseif ( -not ($ObjectName -cmatch '[A-Z]') ) {
        # snake_case
        $TestName = "$($ObjectName)_tests"
    } else {
        # PascalCase or camelCase
        $TestName = "$($ObjectName)Tests"
    }
    # Add newline to pre-test block
    if ($PreTestBlock) { $PreTestBlock += "`r`n" }
